...
async def main(name: str):
    logging.info(f'Starting labels activity that was called by: {name}')
    ...
    async for item in PaginatedResponse(
        endpoint=f'{url}/api/v4.0/labels',
        request_type='GET',
        authentication=authentication_object).items():
        entities_found += 1
        try:
            full_data = {**item, 'sampling_timestamp': sampling_timestamp}
            items_batch.append(GuardicoreLabel(**full_data).model_dump())
            if len(items_batch) >= SENTINEL_BATCH_SIZE:
                logging.info(f"Posting {len(items_batch)} labels to Sentinel")
                await azure_connection.post_data(body=json.dumps(items_batch), log_type='GuardicoreLabels')
                logging.info(f"Posted {len(items_batch)} labels to Sentinel")
                items_batch.clear()
    ...
    logging.info(f"Processed {entities_found} labels")
