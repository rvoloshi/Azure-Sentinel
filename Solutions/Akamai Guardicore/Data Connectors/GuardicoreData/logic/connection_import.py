...
def connection_fetching(azure_connection: AzureSentinel, context: df.DurableOrchestrationContext, context_lock: Lock):
    ...
    for item in PaginatedResponse(
        ...
        try:
            ...
            if len(items_batch) >= SENTINEL_BATCH_SIZE:
                ...
                azure_connection.post_data(body=json.dumps(items_batch), log_type='GuardicoreConnections')
                ...
                with context_lock:
                    yield context.call_entity(datetime_entity_id, "set",
                                              {"type": "last_connection_time", "time": last_connection_time})
                items_batch.clear()
